...
void madc32data::addEvent(TreeData loc_data) { data_vec.push_back(loc_data); }

void madc32data::writeEvent(int evCounter, int highBits) {
  ...
  for (auto i = 0; i < data_vec.size(); i++) {
    ...
    TimeStamp = evCounter + (highBits << 30);
    roottree->Fill();
  }
...
void madc32data::writeTree() { roottree->Write(); }